    ...
    <section class="main-section">
      <nav class="navbar navbar-expand-lg bg-body-tertiary fixed-top" data-bs-theme="light">
        <div class="container-fluid">
          <p class="navbar-brand">Harry Potter and the Goblet of Fire Quiz</p>
          <p id="timer" class="navbar-brand" style="margin-left: 1000px;">05:00</p>
...
<script>
 function startTimer(duration, display) {
    ...
    var storedStartTime = localStorage.getItem('quizStartTime');

    // If storedStartTime is available, calculate the remaining time
    if (storedStartTime) {
      var elapsedTime = Math.floor((Date.now() - parseInt(storedStartTime, 10)) / 1000);
      var remainingTime = duration - elapsedTime;

      // Ensure remaining time is not negative
      var timer = remainingTime >= 0 ? remainingTime : 0;
    } else {
      // If no stored start time, use the default duration
      var timer = duration;
    }
 ...
 document.addEventListener('contextmenu', function (event) {
  ...
  // Add this code where you handle the form submission
  document.getElementById('anssubmit').addEventListener('click', function () {
    // Clear the timer value from localStorage
    localStorage.removeItem('quizTimer');

    // Allow navigation after submitting the quiz
  ...
  window.oncontextmenu = function () {
    return false;
      ...
      <form action="checkanswers.php" method="post">
        ...
        $sql = "SELECT * FROM quesans WHERE book_id = 4 ORDER BY RAND() LIMIT 10";
        ...
        endwhile; ?>
            <div class="col-md-8 mb-5">
              ...
              <center><button type="submit" name="answer-submit" class="sub" id="anssubmit">Submit Answers</button>
              </center>
            </div>
          </div>
        </div>
  ...
  if (isset($_POST['answer-submit'])) {
    // Increment reattempt count
    if (!isset($_SESSION['quiz_reattempt_count'])) {
        $_SESSION['quiz_reattempt_count'] = 1;
    } else {
        $_SESSION['quiz_reattempt_count']++;
    }
